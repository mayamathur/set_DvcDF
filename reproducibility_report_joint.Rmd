...
Group: SE
```{r}
compareValues(reportedValue = 400, obtainedValue = 406.72)
```

Group: t-value
```{r}
compareValues(reportedValue = -.71, obtainedValue = -0.694)
```
...
Block: SE
```{r}
compareValues(reportedValue = 31, obtainedValue = 31.64)
```

Block: t-value
```{r}
compareValues(reportedValue = .62, obtainedValue = 0.608)
```
...
compareValues(reportedValue = 64.03, obtainedValue = block_KR$stats$ddf)
```

Block: p-value
```{r}
compareValues(reportedValue = .54, obtainedValue = block_KR$stats$p.value)
```
...
Group*block: SE
```{r}
compareValues(reportedValue = 44, obtainedValue = 44.74)
```

Group*block: t-value
```{r}
compareValues(reportedValue = -2.01, obtainedValue = -1.975)
```
...
compareValues(reportedValue = 64.03, obtainedValue = interaction_KR$stats$ddf)
```

Group*block: p-value
```{r}
compareValues(reportedValue = .05, obtainedValue = interaction_KR$stats$p.value)
```
