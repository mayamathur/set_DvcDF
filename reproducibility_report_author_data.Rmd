...
[Some useful packages are being loaded below. You can add any additional ones you might need to.]
...
# temporary - change path later
data_file <- "~/Dropbox/Personal computer/Independent studies/Mike's Cognition project on data sharing policy/Co-piloting/Copilot #4/set_DvcDF/data/data.xlsx"
...
```{r}
compareValues(reportedValue = 2380701, obtainedValue = 2274757)
```

Compares calculated variance of block random effect to reported value:
```{r}
compareValues(reportedValue = 26127, obtainedValue = 21928)
```
...
Intercept: SE
```{r}
compareValues(reportedValue = 283, obtainedValue = 282.92)
```

Intercept: t-value
```{r}
compareValues(reportedValue = 11.57, obtainedValue = 11.568)
```

Intercept: DF
```{r, warning=FALSE, message = FALSE}
compareValues(reportedValue = 64.03, obtainedValue = 62.00)
```

Intercept: p-value
```{r}
compareValues(reportedValue = .0001, obtainedValue = 0.0000000000000002)
```

Group: coefficient
```{r}
compareValues(reportedValue = -283, obtainedValue = -282.43)
```

Group: SE
```{r}
compareValues(reportedValue = 400, obtainedValue = 400.11)
```

Group: t-value
```{r}
compareValues(reportedValue = -.71, obtainedValue = -0.706)
```
...
Group: p-value
```{r}
compareValues(reportedValue = .48, obtainedValue = 0.4829)
```

Block: coefficient
```{r}
compareValues(reportedValue = 19, obtainedValue = 19.25)
```

Block: SE
```{r}
compareValues(reportedValue = 31, obtainedValue = 31.12)
```

Block: t-value
```{r}
compareValues(reportedValue = .62, obtainedValue = 0.618)
```
...
Block: p-value
```{r}
compareValues(reportedValue = .54, obtainedValue = 0.5386)
```

Group*block: coefficient
```{r}
compareValues(reportedValue = -88, obtainedValue = -88.35)
```

Group*block: SE
```{r}
compareValues(reportedValue = 44, obtainedValue = 44.02)
```

Group*block: t-value
```{r}
compareValues(reportedValue = -2.01, obtainedValue = -2.007)
```
...
Group*block: p-value
```{r}
compareValues(reportedValue = .05, obtainedValue = 0.0491)
```
...
Ultimately, because there are clearly (per the author) both major and minor numerical errors in the article, we not pursue finding the ultimate reasons for remaining discrepancies. 

Lastly, we note that the article repeatedly refers to interaction as "mediation." These are completely distinct concepts.
